...
visualization_msgs::Marker res_rays_msg;
...
void setup_messaging_interface(ros::NodeHandle &n)
{
	...
	res_rays_publ   = n.advertise<visualization_msgs::Marker>("degub/residual_rays", 10);
...
void loop(const ros::NodeHandle &n)
{
	...
	while (n.ok())
	{
		//cout << "QQ " << debug_mode << endl;
		r.sleep();
		ros::spinOnce();

		// If we still didn't get at least one the lidar data, wait before
		// running the estimator
		if(top_lidar_msg.ranges.size() == 0 ||
		   bot_lidar_msg.ranges.size() == 0)
			continue;
		// If the time interval between the arrivals are more than some certain
		// duration, do not run estimator. This means scans might have been taken
		// from significantly distant locations. Usually lidars run at 20-40Hz.
		// ### Estimator should be able to handle both cases with two scans available
		// and also with one of them.
		if(fabs((top_lidar_msg.header.stamp - bot_lidar_msg.header.stamp).toSec()) > 1.0/20.0)
			continue;
		...
		//cout << "-- Q1" << endl;
		tunnel_localizer.push_lidar_data(top_lidar_msg, top_lidar_calib_params);
		//cout << "-- Q2" << endl;
		...
		//dcm = utils::yaw2dcm(-rpy(2));
		//estm_pose.topLeftCorner(3, 3) = dcm;
		//estm_pose = estm_pose * init_pose;

		//cout << estm_pose.topLeftCorner(3, 3) * estm_pose.topRightCorner(3, 1) << endl;

		//cout << "-- Q4" << endl;
		// ### Should get the estimate before reseting.
		tunnel_localizer.reset();
		//cout << "-- Q5" << endl;
