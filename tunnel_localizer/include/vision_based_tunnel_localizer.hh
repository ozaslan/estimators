...
This class is an implementation of vision based localition estimator.
Thus by definition, it gets a number of inputs, the maps and initial
...
private:
	...
	UniformFeatureExtractor			_extractor;	// is passed to feature tracker. 
	vector<vector<cv::Mat> >		_images;	// ### Do I need these? I will track feats. once the image is given.
	Eigen::Matrix4d					_poses[2];	// poses from RangeBasedTunnelLocalizer.
	vector<CameraCalibParams>		_cam_params;
	vector<Eigen::Matrix3d>			_inv_camera_matrices;

	int _num_frames_pushed;	// counts the number of calls to 'push_image_data(...)' after the last 'estimate_displacement(...)' function call.

	// These vectors are filled with UniformFeatureTracker::get_features(...) function which
	// does memory allocation. Thus frequent calls should be avoided. Tip of the flow vector
	// becomes the tail in the next 'estimate_displacement(...)' call, hence stored in the
	// buffer until overwritten in the next-next estimation.
	int _feat_buffer_idx;						// defined the buffer to be used in the current estimation step.
	...
	int _num_feats;			// # of features to be tracked by each tracker.
	...
	// Estimation results
	double _x_disp;			// displacement estimate along world-x direction.
	...
	// ### Give warnings if estimation is postponed longer than history length.
public:
	// 
	VisionBasedTunnelLocalizer(int num_feats = 50, int history_len = 2, 
								string detector_type = string("FAST"), 
								double threshold = 9);
	// This function calculates the displacement in only world-x direction by utilizing
	// the vector of frames. At every call image features are extracted and
	// tracted from the previous frame. The user might want to estimate the displacement
	// after each frame vector insertion or can wait for '< history_len' before any
	// information is lost. This function incorporates the partial state estimate
	// from RangeBasedTunnelLocalizer which is given through the 'pose' variable.
	// This function assumes that each camera's relative position w.r.t. the platform 
	// is already given through 'register_params(...)' function. Otherwise the function
	// will raise an exception. And the order of the frames is assumed to be given in the 
	// same order at every call. This implicitly assumes that each frame maps to the 
	// same parameter throughout lifetime of the class or until another parameter set is registered.
	bool push_image_data(const vector<cv::Mat> &frames, const Eigen::Matrix4d &pose);
	// This function registers camera calibration parameters. Everytime a new parameter
	// set is registered, 'reset(...)' is called internally. The order of the frames supplied
	...
	bool estimate_displacement(double &x_dist);
	...
	// along the world-x direction. Otherwise last disp. obtained after the most 
	...
	// map. It does not back-project the flow vector but uses the results of
	// the 'estimate_displacement(...)' function. Hence it assumes that the 
	// estimator function has been already called. Otherwise flow vectors
	// from the most recent estimation is returned without further notifications.
	// Output of this function can be used for visualization or further
	// external calculation regarding the estimator accuracy.
	...
	// This function plots the flow field where each trailer is of length '_history_len'.
	// It assumes that the 'img' vector is of same size with 'CameraCalibParams'.
	// Otherwise it returns 'false' without any plots.
	bool plot_flow(vector<cv::Mat> &img, bool plot_flow = true, bool plot_feats = true);
};
