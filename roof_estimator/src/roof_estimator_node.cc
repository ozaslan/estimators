...
LaserProc top_lidar_proc, bot_lidar_proc;
...
void process_inputs(const ros::NodeHandle &n)
{
  ...
  //bottom_lidar_calib_params.relative_pose.topLeftCorner<3, 3>() = bottom_lidar_calib_params.relative_pose.topLeftCorner<3, 3>() * utils::trans::yaw2dcm(1 /180.0 * PI);
  ...
  while (n.ok())
  {
    ...
    if(num_lidar_data == 2){
      ...
      if(turn % 10000 == 0){
        ...
        roof_mapper.register_lidar_data(estm_pose, bot_lidar_proc);
        ...
        cout << "# of pts in last_range_registration : " << temp_cloud.points.size() << endl;
...
void bot_lidar_callback(const sensor_msgs::LaserScan &msg){
  ...
  bot_lidar_proc.update_data(bot_lidar_msg, bottom_lidar_calib_params);

  bot_lidar_proc.median_filter(median_filter_window);
  bot_lidar_proc.cluster(cluster_min_skips, cluster_range_jump, cluster_min_cluster_size);
  bot_lidar_proc.rate_linearity(rate_linearity_window);
  bot_lidar_proc.downsample(downsample_thres);
  bot_lidar_proc.remove_occlusions(remove_occlusion_thres, remove_occlusion_window);
  bot_lidar_proc.extract_lines(extract_lines_min_len, extract_lines_min_pts, extract_lines_epsilon);
  bot_lidar_proc.transform(Eigen::Matrix4d::Identity(), false);
