...
ros::Publisher  odom_publ, laser_pc_publ, octomap_publ, pcmap_publ;
...
int main(int argc, char* argv[]){
	...
	//rbrm  = RangeBasedRoofMapper(0.01);
...
void process_inputs(const ros::NodeHandle &n)
{
	...
	n.param("intensity_filter", intensity_filter, false);
	n.param("intensity_thres", intensity_thres, 1000.0);
	...
	ROS_INFO("[intesity_filter] ---------- : [%s]"  , intensity_filter ? "TRUE" : "FALSE");
	ROS_INFO("[intesity_thres] ----------- : [%.3f]", intensity_thres);
...
void setup_messaging_interface(ros::NodeHandle &n)
{
	...
	laser_pc_publ = n.advertise<sensor_msgs::PointCloud>("debug/pc/laser", 10);
	octomap_publ  = n.advertise<octomap_msgs::Octomap>  ("debug/pc/octomap"  , 10);
	pcmap_publ    = n.advertise<sensor_msgs::PointCloud>("debug/pc/pcmap"  , 10);
}
...
void bottom_lidar_callback(const sensor_msgs::LaserScan &msg){
	...
	if(intensity_filter == true)
		bottom_lidar_proc[step % 2].intensity_filter(intensity_thres);
...
void top_lidar_callback(const sensor_msgs::LaserScan &msg){
	...
	if(intensity_filter == true)
		top_lidar_proc[step % 2].intensity_filter(intensity_thres);
...
void iterate_odom(){
	...
	if(step < 2){
		...
		Eigen::Matrix4d dtrans = prev_trans.inverse() * trans;

		//dtrans = Eigen::Matrix4d::Identity();

		dpose = two_lidars_odom.estimate_odometry(bottom_lidar_proc, top_lidar_proc, dtrans, false, (step % 2) == 0);
		prev_trans = trans;
		// Correct roll-pitch with IMU
		...
		//rbrm.register_scan (pose,    top_lidar_proc[step % 2]);
		//rbrm.register_scan (pose, bottom_lidar_proc[step % 2]);
	}
...
void publish_octomap(){
	...
	octomap_msgs::binaryMapToMsg(rbrm.get_map(), octomap_msg); 
	octomap_publ.publish(octomap_msg);
}
...
void publish_pcmap(){
	...
	pcmap_msg.header.seq++;
	pcmap_msg.header.frame_id = "world";
	pcmap_msg.header.stamp = ros::Time::now();
	...
	if(pcmap_msg.header.seq % 30 == 0)
		pcmap_publ.publish(pcmap_msg);
}
